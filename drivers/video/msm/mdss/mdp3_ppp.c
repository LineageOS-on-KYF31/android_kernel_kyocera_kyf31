...
int mdp3_calc_ppp_res(struct msm_fb_data_type *mfd,  struct blit_req_list *lreq)
{
	...
	if (lreq->req_list[0].flags & MDP_SOLID_FILL) {
		...
		if ((old_solid_fill_pixel >= new_solid_fill_pixel) || (panel_info->yres/2 > req->dst_rect.h) ||
			(mdp3_res->solid_fill_vote_en)) {
			pr_debug("Last fill pixels are higher or fill_en or Solid fill less than H/2 %d\n",
				mdp3_res->solid_fill_vote_en);
			ATRACE_END(__func__);
				return 0;
		}
