...
static const struct msm_camera_i2c_reg_conf eeprom_read_settings[] = {
	{0x0103, 0x01},
	{0x0302, 0x1e},
	{0x0303, 0x00},
	{0x0304, 0x03},
			      ...
			      struct msm_eeprom_memory_block_t *block)
{
	...
	for( j = 0; j < ARRAY_SIZE(eeprom_read_settings); j++)
	{
		rc = e_ctrl->i2c_client.i2c_func_tbl->i2c_write(
			...
			if (rc < 0) {
				pr_err("%s: eeprom read settings write failed. addr[%d]=0x%x, data[%d]=0x%x\n",
					__func__, j, eeprom_read_settings[j].reg_addr,
					j, eeprom_read_settings[j].reg_data);
				return rc;
			}
	}
	msleep(50);
